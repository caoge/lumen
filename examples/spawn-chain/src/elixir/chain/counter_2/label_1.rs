...
fn result(process: &Process, ok: Term, next_pid: Term, output: Term) -> exception::Result<Term> {
    ...
    match received {
        Some(Ok(n)) => {
            process.queue_frame_with_arguments(label_2::frame().with_arguments(false, &[n, next_pid, output]));
        ...
        None => {
            ...
            process.queue_frame_with_arguments(frame().with_arguments(false, &[ok, next_pid, output]));
