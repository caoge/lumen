...
fn result(process: &Process, ok_tr: Term, document: Term, text: Term) -> exception::Result<Term> {
    ...
    let pid_text_binary = process
        .binary_from_str(&format!("{}", process.pid_term()))?;
    process.queue_frame_with_arguments(liblumen_web::document::create_text_node_2::frame().with_arguments(false, &[
        document,
        pid_text_binary,
    ]));

    process.queue_frame_with_arguments(label_4::frame().with_arguments(true, &[document, tr, text]));
