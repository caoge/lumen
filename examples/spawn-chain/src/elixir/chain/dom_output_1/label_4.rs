...
fn result(process: &Process, pid_text: Term, document: Term, tr: Term, text: Term) -> exception::Result<Term> {
    ...
    process.queue_frame_with_arguments(liblumen_web::document::create_element_2::frame().with_arguments(false, &[
        document,
        tag,
    ]));

    process.queue_frame_with_arguments(label_5::frame().with_arguments(true, &[
        document,
        tr,
        pid_text,
        text,
