...
fn result(process: &Process, ok_pid_td: Term, document: Term, tr: Term, pid_text: Term, text: Term) -> Term {
    assert!(ok_pid_td.is_boxed_tuple(), "ok_pid_td ({}) is not a tuple", ok_pid_td);
    ...
    process.queue_frame_with_arguments(liblumen_web::node::append_child_2::frame().with_arguments(false, &[
        pid_td,
        pid_text,
    ]));

    process.queue_frame_with_arguments(label_6::frame().with_arguments(true, &[
        document,
        tr,
        pid_td,
        text,
