...
fn result(process: &Process, element: Term, send_to: Term, output: Term) -> exception::Result<Term> {
    // from arguments
    assert!(element.is_integer());
    assert!(send_to.is_pid());
    // from environment
    assert!(output.is_boxed_function(), "Output ({:?}) is not a function", output);
    ...
    process.queue_frame_with_arguments(erlang::spawn_3::frame().with_arguments(false, &[
        module,
        function,
        arguments,
    ]));
