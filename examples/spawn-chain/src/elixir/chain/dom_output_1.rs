...
pub fn closure(process: &Process) -> Result<Term, Alloc> {
    process.export_closure(super::module(), function(), ARITY, Some(native as *const c_void))
}
...
fn result(process: &Process, text: Term) -> Term {
    process.queue_frame_with_arguments(liblumen_web::window::window_0::frame().with_arguments(false, &[]));
