...
fn result(process: &Process, enumerable: Term, initial: Term, reducer: Term) -> exception::Result<Term> {
    match enumerable.decode().unwrap() {
            ...
            match map.get(atom!("__struct__")) {
                Some(struct_name) => {
                    if struct_name == atom!("Elixir.Range") {
                        ...
                        if first <= last {
                            process.queue_frame_with_arguments(reduce_range_inc_4::frame().with_arguments(false, &[
                                first,
                                last,
                                initial,
                                reducer,
                            ...
                            process.queue_frame_with_arguments(reduce_range_dec_4::frame().with_arguments(false, &[
                                first,
                                last,
                                initial,
                                reducer,
                        ...
                        Err(anyhow!("enumerable ({}) is a struct, but not a Range", enumerable)
                                .into(),
                        )
                    }
                }
        ...
        _ => {
            Err(anyhow!("enumerable ({}) is not a map", enumerable).into())
        }
    }
}
