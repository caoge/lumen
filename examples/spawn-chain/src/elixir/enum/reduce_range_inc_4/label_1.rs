...
fn result(process: &Process, new_first: Term, first: Term, last: Term, acc: Term, reducer: Term) -> exception::Result<Term> {
    ...
    match reducer.decode().unwrap() {
        TypedTerm::Closure(closure) => {
            if closure.arity() == 2 {
                process.queue_frame_with_arguments(closure.frame_with_arguments(
                    false,
                    vec![first, acc],
                ));
        ...
        _ => Err(
            badfun(process, reducer, anyhow!("reducer").into()),
        ),
    }
}
